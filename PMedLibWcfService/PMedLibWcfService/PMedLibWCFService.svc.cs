...
namespace PMedLibWcfService
{
    ...
    public class PMedLibWCFService : IPMedLibWCFService
    {
        ...
        private const string LogFileName = @"PMedLibWCFServer.log";
        //private const string DllDir = @"D:\Uni\GIS\Exam\ASP.NET\PMedLibWcfService\PMedLibWcfService\";
            //private string LogPath;
        private const string ConfigFileName = @"PMedLibWCF.cfg";
        ...
        private bool SelectCapacitated(int _m)
        {
            if (!pm_inited)
                return false;
            ...
            {
                if ((_m > 0) && (m < M) &&
                (!pm.Load(ListOfProblem[selectfile_index], ref m, false)))
                    return false;
                else
                {
                    isloaded = true;
